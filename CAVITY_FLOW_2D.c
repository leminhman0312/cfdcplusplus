//SOLVING CAVITY FLOW, INCOMPRESSIBLE NAVIER STOKE, 2D
...
//double CavityFlow(int nt, int nx, int ny, double u[][ny], double v[][ny], double dx, double dy, double dt, double rho, double nu, double p[][ny], double b[][ny], double pn[][ny],int nit,double vn[][ny], double un[][ny],double onebydt,double dxsquare, double dysquare,double u_wall);

double PlotCavityFlow(int nt, int nx, int ny, double dx, double dy, double dt, double rho, double nu, double b[][ny], double pn[][ny],int nit,double vn[][ny], double un[][ny],double onebydt,double dxsquare, double dysquare,double u_wall);
...
int main(){
	...
	int nx = 41; //step in x
	int ny = 41; //step in y
	int nt = 700; //step in time
	int nit = 100; //iterations
	int c = 1.; 
	...
	//CALCULATE CAVITY FLOW in 2D and Plot
	//CavityFlow(nt,nx,ny,u,v,dx,dy,dt,rho,nu,p,b,pn,nit,vn,un,onebydt,dxsquare,dysquare,u_wall);

	PlotCavityFlow(nt,nx,ny,dx,dy,dt,rho,nu,b,pn,nit,vn,un,onebydt,dxsquare,dysquare,u_wall);
	...
	/*//POPULATE X and Y space vectors
	x_vector(x,nx,dx);
	y_vector(y,ny,dy);
	*/ 
	...
	//MESH GRID
	engEvalString(ep,"[Y X] = meshgrid(Y_matlab,X_matlab);");
	engEvalString(ep,"set(gcf, 'Visible', 'off');"); //hidden plot figure
	engEvalString(ep,"c = contourf(X_matlab,Y_matlab,P_matlab.','linewidth',1);");
	engEvalString(ep,"colorbar('southoutside');");
	engEvalString(ep,"shading interp;");
	engEvalString(ep,"colormap(jet);");
	engEvalString(ep,"hold on;");
	engEvalString(ep,"c = quiver(X_matlab,Y_matlab,V_matlab.',U_matlab.',2);");
	engEvalString(ep,"xlabel('X');");
	engEvalString(ep,"ylabel('Y');");
	engEvalString(ep,"title('Cavity Flow 2D');");
	engEvalString(ep,"axis([0 2 0 2]);");
	engEvalString(ep,"saveas(c,'cavity2d.png');");
	*/ 
...
double CavityFlow(int nt, int nx, int ny, double u[][ny], double v[][ny], double dx, double dy, double dt, double rho, double nu, double p[][ny], double b[][ny], double pn[][ny],int nit,double vn[][ny], double un[][ny],double onebydt,double dxsquare, double dysquare,double u_wall){
	...
	for (int time = 0; time<nt;time++){
		...
		for (int i =1; i<nx-1;i++){
			for (int j = 1; j<ny-1; j++){
				u[i][j] = un[i][j]-(un[i][j]*dt*(forwardbackDiff(un[i][j],un[i-1][j],dx)))-(vn[i][j]*dt*(forwardbackDiff(un[i][j],un[i][j-1],dy)))-((dt/(rho))*(centralDiffX(p[i+1][j],p[i-1][j],dx)))+((nu*dt)*(secondOrderCentral(un[i+1][j],un[i][j],un[i-1][j],dx)))+((nu*dt)*(secondOrderCentral(un[i][j+1],un[i][j],un[i][j-1],dy)));
...
double PlotCavityFlow(int nt, int nx, int ny,double dx, double dy, double dt, double rho, double nu, double b[][ny], double pn[][ny],int nit,double vn[][ny], double un[][ny],double onebydt,double dxsquare, double dysquare,double u_wall){
	...
	//Open Videos
	engEvalString(ep,"v = VideoWriter('2D_CAVITY_FLOW.avi');");
